...
# Spinner animado — executa comando em background e anima até terminar
spinner() {
    local label="$1"; shift
    local frames=('⠋' '⠙' '⠹' '⠸' '⠼' '⠴' '⠦' '⠧' '⠇' '⠏')
    local i=0

    # Roda o comando e captura saída/erro
    ...
    # Expõe stdout do comando ao caller via variável global _SPINNER_OUT
...
main() {
    ...
    mkv_original=$(find "$pasta" -maxdepth 1 -type f -name "*.mkv" | head -n 1)
    legenda=$(find "$pasta" -maxdepth 1 -type f -name "*.ass" | head -n 1)
    capitulos=$(find "$pasta" -maxdepth 1 -type f -name "*.txt" | head -n 1)
    ...
    spinner "Calculando CRC-32..." \
        bash -c "$(declare -f calc_crc32); _hash=\$(calc_crc32 '$mkv_temp'); echo \"\$_hash\""
    local hash="$_SPINNER_OUT"
    ...
    spinner "Gerando thumbnail..." \
        ffmpeg -ss 00:00:30 -i "$mkv_final" -vf "thumbnail,setsar=1" -vframes 1 "$thumb" -y
